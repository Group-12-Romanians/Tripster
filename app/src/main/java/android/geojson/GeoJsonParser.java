...
/* package */ class GeoJsonParser {
     ...
     * @throws JSONException if coordinates cannot be parsed
     */
    private static android.geojson.GeoJsonLineString createLineString(JSONArray coordinates) throws JSONException {
        return new android.geojson.GeoJsonLineString(parseCoordinatesArray(coordinates));
    }
    ...
    private static GeoJsonMultiLineString createMultiLineString(JSONArray coordinates)
            throws JSONException {
        ArrayList<android.geojson.GeoJsonLineString> geoJsonLineStrings = new ArrayList<GeoJsonLineString>();
        for (int i = 0; i < coordinates.length(); i++) {
